[CmdletBinding()]
param(
    [uri] $GraphQlEndpoint = 'https://edge.sitecorecloud.io/api/graphql/v1',

    [string] $ApiKey = '<YOUR_XM_CLOUD_API_KEY>',
...
# Raw filtered MergedCards.json data embedded directly for Sitecore PowerShell ISE.
$MergedCardsJson = @'
[
    {
...
function Find-XmCloudItemByItemId {
    param([Parameter(Mandatory)][string] $ItemId)

    $query = @'
query FindItemByItemId(
  $fieldName: String!
  $itemId: String!
  $language: String!
) {
  search(
    where: {
      AND: [
        { name: $fieldName, value: $itemId, operator: EQ }
    ...
    results {
      ...
      name
      path
      matchedItemId: field(name: $fieldName) {
        value
      }
    }
  }
...
'@
    ...
    if ($response.errors) {
        $errorMessages = @(
            $response.errors | ForEach-Object { [string] $_.message }
        ) -join '; '
        throw "XM Cloud GraphQL returned an error for itemid '$ItemId': $errorMessages"
    }

    # XM Cloud EQ can behave like CONTAINS for strings, so enforce exact equality here.
    $exactMatches = @(
        $response.data.search.results |
            Where-Object {
                [string] $_.matchedItemId.value -ceq $ItemId
            }
    )

    if ($exactMatches.Count -gt 1) {
        throw "More than one Sitecore item has itemid '$ItemId'."
    }
...
try {
    if ([string]::IsNullOrWhiteSpace($ApiKey) -or
        $ApiKey -eq '<YOUR_XM_CLOUD_API_KEY>') {
        throw 'Set $ApiKey to your XM Cloud Preview or Delivery API key.'
    }

    # Windows PowerShell 5.1 treats a UTF-8 .ps1 without a BOM as ANSI.
    # Repair that case while leaving Sitecore's correctly decoded script text untouched.
    $mojibakeMarker = -join ([char[]] @(0x00E2, 0x20AC, 0x2122))
    if ($MergedCardsJson.Contains($mojibakeMarker)) {
        $MergedCardsJson = [System.Text.Encoding]::UTF8.GetString(
            [System.Text.Encoding]::Default.GetBytes($MergedCardsJson))
    }

    $parsed = $MergedCardsJson | ConvertFrom-Json
    $records = @(foreach ($record in $parsed) { $record })

    Write-Verbose "Reading $($records.Count) embedded record(s)."
    ...
    foreach ($record in $records) {
        ...
        $sitecoreItem = Find-XmCloudItemByItemId -ItemId $itemId
        if ($null -eq $sitecoreItem) {
            Write-Warning "No Sitecore item found with itemid '$itemId'."
            continue
        }
